...
export function createProjectActions(store: StoreApi<AppStore>, ctx: ServiceContext): ProjectActions {
  return {
    addProject: async (id, projectPath) => {
      // Validate ID
      if (!id.trim()) throw new Error('Project ID is required');
      if (!/^[a-zA-Z0-9-_]+$/.test(id)) throw new Error('Project ID must contain only letters, numbers, hyphens, and underscores');
      ...
      if (!ctx.fileOps.pathExists(expandedPath)) throw new Error(`Path does not exist: ${expandedPath}`);
