...
describe('StepIndicator', () => {
  it('renders all steps with correct labels', () => {
    const { lastFrame } = render(
      <StepIndicator steps={['Step One', 'Step Two', 'Step Three']} currentStep={0} />
    );
  ...
  it('renders current step with > prefix and cyan-bold styling', () => {
    const { lastFrame } = render(
      <StepIndicator steps={['Select Operation', 'Select Skills', 'Confirm']} currentStep={1} />
    );
    const output = lastFrame();
  ...
  it('renders completed steps with green styling', () => {
    const { lastFrame } = render(
      <StepIndicator steps={['Step A', 'Step B', 'Step C']} currentStep={2} />
    );
    const output = lastFrame();
  ...
  it('renders future steps with dim styling', () => {
    const { lastFrame } = render(
      <StepIndicator steps={['Step X', 'Step Y', 'Step Z']} currentStep={0} />
    );
    const output = lastFrame();
